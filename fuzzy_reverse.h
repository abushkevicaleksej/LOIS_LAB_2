	...
	for (size_t j = 0; j < matrix[0].size(); j++) {
		...
		for (size_t k = 0; k < matrix.size(); k++) {
			...
			if (matrix[k][j].second == array[j].second) {
				...
				higher = 1.0;  // Correctly set to 1.0
			}
			else if (matrix[k][j].second > array[j].second) {
				lower = array[j].second;
				higher = 1.0;  // Set to 1.0 instead of array[j].second
			}
            ...
            for (int t = 0; t < solutions[j][k].size(); t++) {
                ...
                for (int i = 1; i < solutions.size(); ++i) {
                    ...
                    for (int s = 0; s < solutions[i].size(); ++s) {
                        ...
                        if (max(lower, lower_to_be_compared) <= min(higher, higher_to_be_compared)) {
                            lower = max(lower, lower_to_be_compared);
                            higher = min(higher, higher_to_be_compared);
